...
class ext extends \phpbb\extension\base
{
	...
	public function is_enableable()
	{
		...
		if (phpbb_version_compare(PHPBB_VERSION, '3.2.0', '<'))
		{
			...
			$language->add_lang_ext('mot/failedlogins', 'mot_fl_ext_enable_error');
		}
		else
		{
			$language = $this->container->get('language');
			$language->add_lang('mot_fl_ext_enable_error', 'mot/failedlogins');
		}
		$ext_name = $language->lang('MOT_FL_EXT_NAME');

		// Check requirements
		if (!$this->phpbb_requirement())
		{
			$this->error_message[] = $language->lang('MOT_FL_ERROR_MESSAGE_PHPBB_VERSION', $this->phpbb_min_ver, $this->phpbb_below_ver);
		}

		if (!$this->php_requirement())
		{
			$this->error_message[] = $language->lang('MOT_FL_PHP_VERSION_ERROR', $this->php_min_ver, $this->php_below_ver);
		}

		if (!empty($this->error_message))
		{
			// Insert general message at the beginning
			array_unshift($this->error_message, $language->lang('MOT_FL_ERROR_EXTENSION_NOT_ENABLE', $ext_name));
